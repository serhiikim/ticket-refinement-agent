...
2. **`src/eventFilter.ts`** → Gates which events trigger processing:
...
3. **`src/contextBuilder.ts`** → Fetches issue + all comments from GitHub API, builds Claude prompt
4. **`src/claudeRunner.ts`** → Runs `claude --print --output-format json` as subprocess; git-resets local repo clone to latest before each run
5. **`src/actionHandler.ts`** → Posts comments, swaps labels, patches issue body, optionally creates draft PRs
...
> **Note**: The systemd unit file on the server must reference `tsx src/index.ts` (not root-level `index.ts`).
